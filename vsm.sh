...
# Usage:
#   ./vsm.sh <INDEX_DIR> <QUERY_FILE_PATH> <OUTPUT_DIR> <STOPWORDS_FILE> <k> [--time]
# Writes TREC-style output to <OUTPUT_DIR>/vsm_docids.txt
# Uses query field: title only

if [ "$#" -lt 5 ] || [ "$#" -gt 6 ]; then
  echo "Usage: $0 <INDEX_DIR> <QUERY_FILE_PATH> <OUTPUT_DIR> <STOPWORDS_FILE> <k> [--time]"
  exit 1
fi
...
RUN_CMD=(python3 "$(dirname "$0")/vsm.py" "$INDEX_DIR" "$QUERY_FILE" "$OUT_DIR" "$STOPWORDS" "$K")
