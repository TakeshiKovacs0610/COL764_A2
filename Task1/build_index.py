...
def build_index(corpus_dir: str, vocab_path: str, doc_limit: int = None) -> InvertedIndex:
    """
    ...
    nlp = _init_tokenizer()
    seen_docs = set()
    field_order = ["title", "doi", "date", "abstract"]
    ...
    for fname in os.listdir(corpus_dir):
        ...
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if not ext_doc_id or ext_doc_id in seen_docs:
                    continue
...
if __name__ == "__main__":
    ...
    inv = build_index(corpus_dir, vocab_file,1000)
    save_index(inv, index_dir)
    save_vsm_index(inv, index_dir)
    print(f"Wrote index.json and vsm.json to: {index_dir}")