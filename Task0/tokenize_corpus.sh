#!/bin/bash
# Usage: ./tokenize_corpus.sh <CORPUS_DIR_OR_FILE> <STOPWORDS_FILE> <VOCAB_DIR> [--time]
# Calls tokenize_corpus.py with the given arguments.

if [ "$#" -lt 3 ] || [ "$#" -gt 4 ]; then
    echo "Usage: $0 <CORPUS_DIR_OR_FILE> <STOPWORDS_FILE> <VOCAB_DIR> [--time]"
    exit 1
fi
